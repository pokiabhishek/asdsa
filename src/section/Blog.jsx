  ...
  return (
    ...
    <div className="container mx-auto px-4">
      ...
      <div className="text-center mb-12">
        <span className="text-lg font-medium text-primary mb-2 block">{subTitle}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
        {blogList.map((val, i) => (
          ...
          >
            ...
            <div className="p-6">
              <a to="/blog-single">
                <h4 className={`text-2xl tracking-tight font-semibold text-black transition-colors duration-300 mb-4 ${hover === i ? "text-primary" : "hover:text-primary"}`}>
                  {val.title}
                </h4>
              </a>
              <div className="flex justify-between text-sm text-gray-600 mb-4">
                <span className="flex items-center gap-2 text-[16px]">
                  <FaUser className='text-primary text-lg'/> {val.author}
                </span>
                <span className="flex items-center gap-2 text-sm tracking-normal">
                  <BsCalendar2DateFill className='text-primary text-lg'/> {val.date}
                </span>
              </div>
              <p className="text-gray-600 mb-4 tracking-normal text-lg">{val.desc}</p>
              <div className="flex justify-between items-center text-sm border-t-2 pt-3">
                <div>
                  <a
                    to="/blog-single"
                    className="text-gray-600 font-semibold text-lg hover:underline flex items-center gap-2"
                  >
                    {val.btnText}
